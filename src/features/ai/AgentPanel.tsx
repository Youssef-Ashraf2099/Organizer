...
export const AgentPanel = ({
  ...
  const handleSendMessage = async (text: string = customPrompt) => {
    ...
    try {
      ...
      // Trim & sanitize history to prevent hallucination on 2nd+ request
      const rawHistory = activeConv
        ? activeConv.messages.map((m) => ({
            role: m.role,
            content: aiService.sanitizeHistoryMessage(m.content, m.role),
          }))
        : [];
      const historyMsgs = aiService.trimHistory(rawHistory);
      ...
      if (mode === "agent") {
        ...
        messages.push({
          role: "system",
          content:
            "You are a helpful AI assistant called Omni AI. Answer the user's questions clearly and concisely. Use markdown formatting. Do NOT output any JSON tool blocks.",
        });
      }
            ...
            <div className="flex-1 overflow-y-scroll p-4 space-y-4 scroll-smooth custom-scrollbar min-h-0">
              ...
              {isLoading && (
                <div className="flex gap-3 justify-start">
                  ...
                  <div className="bg-zinc-800 px-4 py-3 rounded-2xl rounded-tl-sm border border-white/5">
                    <div className="flex items-center gap-2">
