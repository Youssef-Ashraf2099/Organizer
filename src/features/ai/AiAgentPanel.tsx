...
const IconUndo = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5">
    <path d="M3 7v6h6" /><path d="M3 13C5.33 7.67 10.17 4 16 4a9 9 0 0 1 0 18" />
  </svg>
);
const IconX = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
  ...
  <span className="inline-flex items-center gap-1">
    {[0, 1, 2].map((i) => (
      <span
        ...
        style={{ animation: `aiDotBounce 1.2s ease-in-out ${i * 0.2}s infinite` }}
      />
    ))}
  </span>
);
...
const PendingChangeCard = ({ change }: { change: PendingChange }) => {
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-xs text-emerald-300 truncate">{change.description}</span>
      <button
...
export const AiAgentPanel = () => {
  const {
    isOpen, messages, status, errorMessage, pendingChanges, mode, modelState, responseStartedAt,
    sendMessage, acceptChange, undoChange, clearMessages, dismissError, setMode, refreshHealth, cancelRequest,
  } = useAiAgent();
  ...
  const elapsedLabel = isProcessing ? ` · ${Math.max(1, Math.round(elapsedMs / 1000))}s` : "";
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-800 flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-sm font-semibold text-white">Omni AI Agent</h2>
              <p className="text-[11px] text-zinc-500 truncate">
                {status === "syncing" ? `📡 Syncing context${elapsedLabel}` :
                 status === "thinking" ? `🧠 Engine thinking${elapsedLabel}` :
                 status === "error" ? "❌ Error" :
          ...
          {mode === "agent" && (
            <div className="px-4 py-1.5 border-b border-violet-500/20 bg-violet-950/20 flex-shrink-0">
              <p className="text-[11px] text-violet-300">
                🤖 <strong>Agent mode</strong> — uses LangGraph (Researcher → Writer → Reviewer). Best for autonomous tasks.
              </p>
            </div>
          )}
              ...
              >
                <div className="px-3 py-2 flex flex-col gap-2">
                  ...
                  <div className="flex gap-2 mt-1">
                    <button
                      id="ai-accept-all"
                      onClick={() => pendingChanges.forEach((c) => acceptChange(c.id))}
                    ...
                    <button
                      id="ai-undo-all"
                      onClick={() => pendingChanges.forEach((c) => undoChange(c.id))}
            ...
            {modelState === "loading" && (
              ...
              >
                <div className="px-3 py-2 flex items-center gap-2">
                  <span className="text-blue-300 text-xs">☕ Warming up the model for faster replies...</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            {errorMessage && (
              ...
              >
                <div className="flex items-start gap-2 px-3 py-2">
                  ...
                  <button onClick={dismissError} className="text-red-400 hover:text-red-200 transition-colors flex-shrink-0">
                    <IconX />
                  </button>
                </div>
              </motion.div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-4 min-h-0 custom-scrollbar">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-6 text-center px-4">
                ...
                <div>
                  <h3 className="text-base font-semibold text-white mb-1">Omni AI Agent</h3>
                  <p className="text-sm text-zinc-500">
                    Powered by your local engine at <code className="text-zinc-400 bg-zinc-800 px-1 rounded">:8000</code>.
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1 relative">
                <textarea
                  ...
                  placeholder={isProcessing ? "Engine is processing..." : mode === "agent" ? "Describe a task for the agent..." : "Ask the AI agent..."}
                  ...
                  className={cn(
                    ...
                    isProcessing ? "border-zinc-700 opacity-60 cursor-not-allowed" : "border-zinc-700"
                  )}
