...
export const AppLayout = () => {
  ...
  const { state, actions, isOllamaRunning, openPanel, closePanel, executeAction, setSelectedText } =
    useAgentPanel();
    ...
    <div className="h-screen w-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 flex overflow-hidden">
      ...
      <AgentPanel
        ...
        onExecuteAction={(action, selectionOverride, pageContext) =>
          executeAction('current-page', action, selectionOverride, pageContext)
        }
        onInsertResponse={(response) => {
          // Trigger insertion in editor
          const event = new CustomEvent('insertAIResponse', { detail: { response } });
