...
export const CursorEffect = () => {
  ...
  }, [mouseX, mouseY, isVisible, rotateX, rotateY]);
  ...
  const PointerSVG = ({ color = "white", outline = "black" }) => (
    <svg 
      ...
      width="28" height="28" viewBox="-2 -2 28 28" 
  ...
  const renderClassic = () => (
    <motion.div style={{ x: mouseX, y: mouseY, scale: isClicking ? 0.9 : 1 }}>
  ...
  const renderVoxel = (variant: "normal" | "crystal" = "normal") => (
    <motion.div
      style={{
        x: mouseX, y: mouseY, transformStyle: "preserve-3d",
        rotateX: springRotateX, rotateY: springRotateY,
        scale: isClicking ? 0.85 : 1,
      }}
    >
      {[...Array(variant === "crystal" ? 12 : 8)].map((_, i) => (
        <div key={i} className="absolute" style={{ transform: `translateZ(${-i * 1.2}px)`, opacity: 1 - (i * 0.08) }}>
          {isHovering ? (
            <HandSVG 
              color={variant === "crystal" ? (i === 0 ? "#e879f9" : "#701a75") : (i === 0 ? "white" : "#d4d4d8")} 
              outline={variant === "crystal" ? "#2e1065" : "black"} 
            />
          ) : (
            <PointerSVG 
              color={variant === "crystal" ? (i === 0 ? "#818cf8" : "#312e81") : (i === 0 ? "white" : "#d4d4d8")} 
    ...
    <motion.div style={{ x: mouseX, y: mouseY, scale: isClicking ? 0.9 : 1 }}>
       <div className="relative">
         ...
         <div style={{ filter: "drop-shadow(0 0 8px rgba(168, 85, 247, 0.5))" }}>
           {isHovering ? <HandSVG color="transparent" outline="url(#neonGrad)" /> : <PointerSVG color="transparent" outline="url(#neonGrad)" />}
         </div>
  ...
  const renderGlitch = () => (
    <motion.div style={{ x: mouseX, y: mouseY }}>
      <div className="relative">
        <motion.div className="absolute top-0 left-0" animate={{ x: [-3, 3, -2], y: [2, -2, 3] }} transition={{ repeat: Infinity, duration: 0.15 }}>
          {isHovering ? <HandSVG color="#ff0080" outline="none" /> : <PointerSVG color="#ff0080" outline="none" />}
        </motion.div>
        <motion.div className="absolute top-0 left-0" animate={{ x: [3, -3, 2], y: [-2, 2, -3] }} transition={{ repeat: Infinity, duration: 0.15, delay: 0.05 }}>
          {isHovering ? <HandSVG color="#00fff0" outline="none" /> : <PointerSVG color="#00fff0" outline="none" />}
        </motion.div>
        <div className="relative z-10">
          {isHovering ? <HandSVG color="white" outline="black" /> : <PointerSVG color="white" outline="black" />}
        </div>
      </div>
    </motion.div>
  );
  ...
  const renderCircle = () => (
    <>
      <motion.div className="fixed w-2 h-2 bg-blue-500 rounded-full" style={{ x: mouseX, y: mouseY, translateX: "-50%", translateY: "-50%" }} />
      <motion.div
        className="fixed rounded-full border border-blue-400/50"
        style={{
          x: posX, y: posY, translateX: "-50%", translateY: "-50%",
          width: isHovering ? 60 : 32, height: isHovering ? 60 : 32,
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[99999] no-print" style={{ perspective: "1000px" }}>
